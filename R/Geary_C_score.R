...
Geary_C_score <- function(Expression_file, Meta_data, Output_path, Method, Tissue, pvalue_threshold = 0.01) {
  ...
  spatial_weight_matrix@x <- 1 / spatial_weight_matrix@x
  ...
  compute_geary <- function(i) {
    ...
    c(gearys_C = gearys_C, p_value = p_value)
  }
  
  results <- t(sapply(1:ncol(Expression_file), compute_geary))
  
  geary_vector <- 1 / results[, "gearys_C"]
  p_value_vector <- results[, "p_value"]

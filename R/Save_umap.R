...
Save_umap <- function(Umap_result,object = Clustering,Output_path,Method,Tissue,name_object = 'Clustering') {
  optimal_palette <- rainbow(length(unique(object)))
  names(optimal_palette) <- unique(object)
  png(paste0(Output_path, Method, "_", Tissue,'_', name_object, "_umap.png"), width = 1000, height = 1000)
